                ...
                <td class="wrapper">
                  <table border="0" cellpadding="0" cellspacing="0">
                    <tr>
                      <td>
						...
						<table class="f555">
							<tr>
								<td class="w25"><strong>Order placed on:</td>
								<td>{{ date('M d, Y', strtotime($data->orderHeader->created_at))}}</td>
							</tr>
							<tr>
								<td class="w25"><strong>Order number:</td>
								<td>{{$data->orderHeader->order_number}}</td>
							</tr>
							<tr>
								<td class="w25"><strong>Invoice number:</td>
				  ...
				  <table border="0" cellpadding="0" cellspacing="0">
					<tr class="thead">
						...
						<td class="w20">Size</td>
						<td class="no-bg w2"></td>
						<td class="w17">QTY Invoiced</td>
						<td class="no-bg w2"></td>
						<td class="w32">Amount Invoiced</td>
					</tr>
					<tbody class="tbody">
					    @foreach ($data->invoiceDetail as $id)
						<tr>
							<td><p><strong>{{$id->orderDetail->item->name . '-' . $id->orderDetail->item->short_description}} </strong></p></td>
							<td></td>
							<td><p><strong>{{count($id->orderDetail->optionSize) >=1 ? $id->orderDetail->optionSize->name : 'N/A'}}</strong></p></td>
							<td></td>
							<td><p><strong>{{$id->qty}}</strong></p></td>
							<td></td>
							<td><p><strong>{{$id->amount}}</strong></p></td>
						</tr>
					    @endforeach
						...
						<tr>
							...
							<td colspan="5" class="black">
								<table>
									<tr><td><p  class="head"><strong>Order Details</strong></p></td></tr>
									<tr><td>&nbsp;</td></tr>
									<tr>
										<td class="p0 w64"><p><strong>Subtotal: </strong></p></td>
										<td><p>PHP {{ Helper::numberFormat($data->orderHeader->subtotal)}}</p></td>
									</tr>
									<tr>
                                        <td class="p0"><p><strong>Shipping Rate: </strong></p></td>
										<td><p>PHP {{  Helper::numberFormat($data->orderHeader->shipping_rate)}}</p></td>
									</tr>
									<tr>
										<td class="p0"><p><strong>Discount applied: </strong></p></td>
										<td><p>PHP {{  Helper::numberFormat($data->orderHeader->discount)}}</p></td>
									</tr>
									<tr>
										<td class="p0"><p class="big"><strong>Your order's grand total is: </strong></p></td>
										<td><p class="big"><strong>PHP {{  Helper::numberFormat($data->orderHeader->grand_total)}}</strong></p></td>
									</tr>
									<tr><td>&nbsp;</td></tr>
								</table>
							</td>
